...
- `standardWidth` and `standardHeight` - the default width and height of the texture. These are required so that higher resolution textures will work. These are not very useful for the skin input because skins should always be 64x64, but I left them here until I have to add another argument to the skin input.
- `choices` - an array of texture names that may be selected instead of selecting a texture file. Specify an empty array `[]` if none required. The default Minecraft skins are included in `MinecraftSkins.skins`, and they can be used without adding them to a generator's texture folder.
...
Drawing a cuboid happens frequently when using Minecraft textures, as almost all parts of a model are six-sided cuboids and are organized as such on the texture. `Minecraft.drawCuboid()` is provided to make drawing multiple cuboids simpler and less confusing.

To do this, you need to:

1. Identify the x, y coordinates plus width, height and depth of a cuboid on the texture to copy
2. Identify the x, y coordinates plus width, height and depth of a cuboid on the page to draw it into

For example, suppose we want to copy the head of a Minecraft skin. 

![Texture with part of the image highlighted](./static/draw-texture-skin.png)

This is a cuboid (a cube in this case) with the following coordinates and size:
